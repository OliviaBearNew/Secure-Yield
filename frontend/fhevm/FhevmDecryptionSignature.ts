...
export class FhevmDecryptionSignature {
  ...
  static checkIs(s: unknown): s is FhevmDecryptionSignatureType {
    ...
    if (!("domain" in (s as any).eip712 && typeof (s as any).eip712.domain === "object")) return false;
    if (!("primaryType" in (s as any).eip712 && typeof (s as any).eip712.primaryType === "string")) return false;
    if (!("message" in (s as any).eip712)) return false;
    if (!("types" in (s as any).eip712 && typeof (s as any).eip712.types === "object" && (s as any).eip712.types !== null)) return false;
    return true;
  }
